...
class zonemap {
  ...
  unsigned int num_zones;  // keeps track of the number of zones/partitions to
                           // the array/vector of elements
  unsigned int num_elements_per_zone;  // keeps track of number of elements per
                                       // zone (items in every zone has to be
                                       // less than or equal to this counter)
 ...
 public:
   ...
   * returns: N/A
   */
  zonemap(std::vector<T> _elements, unsigned int _num_elements_per_zone);
